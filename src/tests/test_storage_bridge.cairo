...
mod tests {
    ...
    use warpack_masters::models::Character::{WMClass, m_Character, m_CharacterName};
    ...
    use warpack_masters::models::TokenRegistry::m_TokenRegistry;
    ...
    use warpack_masters::systems::storage_bridge::{
        IStorageBridgeDispatcher, IStorageBridgeDispatcherTrait, storage_bridge,
    };
    ...
    fn create_token_and_setup_storage(
        alice: ContractAddress,
        default_address: ContractAddress,
        item_id: u32,
        token_factory: ITokenFactoryDispatcher,
        ...
        let storage_counter = StorageCounter { player: alice, count: 1 };
    ...
    fn test_deposit_item_success() {
        let (alice, default_address, storage_bridge, token_factory, actions, mut world) =
            setup_test_environment();
        let item_id = 6; // Dagger

        let token_address = create_token_and_setup_storage(
            alice, default_address, item_id, token_factory, actions, world,
        );
        ...
        token_contract
            .transfer(storage_bridge.contract_address, token_amount * 10); // Give it some tokens

        let player_balance = token_contract.balance_of(alice);
        assert(
            player_balance == TOKEN_SUPPLY_BASE - token_amount * 10, 'Player should have tokens',
        );
        ...
        let player_balance = token_contract.balance_of(alice);
        assert(
            player_balance == TOKEN_SUPPLY_BASE - token_amount * 9, 'Player should receive tokens',
        );
    }
    ...
    fn test_withdraw_item_success() {
        ...
        set_contract_address(default_address);
        let storage_counter: StorageCounter = world.read_model(alice);
    ...
    fn test_deposit_item_nonexistent_storage_item() {
        ...
        create_token_and_setup_storage(
            alice, default_address, item_id, token_factory, actions, world,
        );
    ...
    fn test_deposit_item_token_not_registered() {
        let (alice, default_address, storage_bridge, _, actions, mut world) =
            setup_test_environment();
