...
pub trait IItem<T> {
    ...
    fn batch_add_items(ref self: T);

    fn update_item_enabled(ref self: T, id: u32, enabled: bool);
}
...
mod item_system {
    ...
    impl ItemImpl of IItem<ContractState> {
        ...
        fn batch_add_items(ref self: ContractState) {
            let mut world = self.world(@"Warpacks");
        ...
        fn update_item_enabled(ref self: ContractState, id: u32, enabled: bool) {
            let mut world = self.world(@"Warpacks");
            let player = get_caller_address();
